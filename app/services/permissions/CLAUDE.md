...
`user_context` es un objeto que conoce: `user`, `current_church`, `church_user`, `role_assignments`, `assigned_ministry_ids`.
...
3. **Rol pastoral.** Para que `allow?(module_key: "pastoral_notes", ...)` devuelva `true`, **todos** estos deben cumplirse:
   - El usuario tiene al menos un `RoleAssignment` activo a un `Role` con `pastoral: true`.
   - Ese rol tiene un `RolePermission` activo sobre el módulo `pastoral_notes` con la acción solicitada.
4. **Owner bootstrap.** Si `church_user.owner == true`, las acciones admin estándar (todas excepto `pastoral_notes.*`) devuelven `true` aunque no haya roles configurados. Esto es excepción explícita, documentar siempre.
...
7. **Acciones derivadas.** `read_only` implica `read/list/show`. `read_write` añade `create/update`. `full_access` añade `activate/deactivate`. `export` y `manage` son siempre checkboxes independientes — no caen en los atajos.

## Cómo agregar un módulo nuevo

1. Agregar registro a `permission_modules` en seeds (`db/seeds.rb` o seed dedicado), con `key`, `name`, `category`, `position`.
...
1. Agregar columna `can_<accion>` a `role_permissions` (migración).
2. Actualizar `RolePermission` (validación + atajos si aplica).
...
- Cargar `RoleAssignment` + `RolePermission` del usuario una sola vez por request (cachear en `user_context`).
